...
const initialState = {
  tdDefaults: [
    ...
    },
    {
      ...
      tdNumber: 980,
      section: "functional_package_for_transport_layer_security_tls_21",
      cc_ids: ["FCS_TLSS_EXT.1.4", "FCS_TLSC_EXT.1.4", "FCS_DTLSS_EXT.1.4", "FCS_DTLSC_EXT.1.4"],
      sfrs: [],
    },
    {
...
export const tdSlice = createSlice({
  ...
  reducers: {
    CREATE_TD: (state, action) => {
      let newId = uuidv4();
      const { title, tdNumber, tdReason, uniquePP, sfrs, applied } = action.payload;
      ...
      if (!match && !state.hasOwnProperty(newId)) {
        ...
        action.payload = { uuid: newId, tdReason: tdReason, tdNumber: tdNumber, uniquePP: uniquePP, applied: applied || "Yes" };
      } else {
        if (match) {
          const [matchedKey, matchedValue] = match;
          let tdReason = matchedValue.tdReason;
          let applied = matchedValue.applied || "Yes";
          action.payload = { uuid: matchedKey, tdReason: tdReason, tdNumber: tdNumber, uniquePP: uniquePP, applied: applied };
        } else {
          action.payload = null;
        }
      }
    ...
    UPDATE_TD: (state, action) => {
      const { uuid, tdReason, applied } = action.payload;
      if (state.hasOwnProperty(uuid)) {
        state[uuid].tdReason = tdReason;
        if (applied !== undefined) state[uuid].applied = applied;
        action.payload = { uuid: uuid, tdReason: tdReason, tdNumber: state[uuid].tdNumber, uniquePP: state[uuid].uniquePP, applied: state[uuid].applied };
      }
    },
...
export const { ADD_SFR_TO_TD_DEFAULT, CREATE_TD, UPDATE_TD, DELETE_TD, RESET_TD_STATE } = tdSlice.actions;
